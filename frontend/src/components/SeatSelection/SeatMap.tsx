...
const SeatMap = ({ totalRows, seatsPerRow, selectedSeats, occupiedSeats, onSeatClick }: SeatMapProps) => {
    return (
        <div className="flex flex-col items-center select-none overflow-x-auto w-full pb-4 scrollbar-thin">
            {/* Screen Banner */}
            ...
            {/* Seat Grid */}
            <div className="space-y-2 md:space-y-3 min-w-max px-4">
                {[...Array(totalRows)].map((_, r) => {
                    const rowLabel = String.fromCharCode(65 + r); // A, B, C...
                    return (
                        <div key={rowLabel} className="flex items-center gap-3 md:gap-4 justify-center">
                            {/* Row Label (Left) */}
                            <span className="text-xs font-bold text-gray-600 w-4 text-center">{rowLabel}</span>
